...
/* put a process into a utsspace */
int mib_addutsnewprocess(int pid, int utsid)
{

	if (utsid == 0)
		return 0; /* space zero is ignored */

	int f_puid = mib_getfreeprocutsid();
	_proc_uts[f_puid].endpt = pid;
	_proc_uts[f_puid].utsid = utsid;
	utsspace[utsid]++;

	return 0;
}

/* 	remove a process from its utsspace. 
	if no process left , the uts space will be released */
int mib_deleteprocessinuts(int endpt)
{
	int utsid = 0;
	...
	for (int i = 0; i < NR_PROCS; i++)
	{
		...
		{
			utsid = _proc_uts[i].utsid;

			_proc_uts[i].utsid = 0;
			_proc_uts[i].endpt = 0;

			utsspace[utsid]--;
			/* utsspace and hostname_uts */
			if (utsspace[utsid] == 0)
			{
				int j = 0;
				while (hosname_uts[utsid][j] != '\0')
				{
					hostname_uts[utsid][j++] = '\0';
				}
			}
			else if (utsspace[utsid] < 0)
			{
				// return error
			}
...
int mib_createutsandaddnewprocess(int p_utsid, endpoint_t c_endpt)
{
	int f_puid = mib_getfreeprocutsid();
	if (f_puid == -1)
		return ENOMEM;
	...
	int ifree = 1;
	while (utsspace[ifree] != 0 && ifree < MAXUTSSPACES)
		ifree++; /* search for the first free item */
	_proc_uts[f_puid].utsid = ifree;

	/* set new hostname */
	if (p_utsid == 0)
	{ /* case for parent process in zero uts namespace */
...
void mib_kern_init(struct mib_node *node)
{
	...
	// initialize process and uts map table
